...
class DatabaseWindow:
    def __init__(self, root, port):
        ...
        self.client = client.Client(nickname, port)
        ...
        self.context_menu.add_command(label="Connect", command=self.connect_to_user)
        self.context_menu.add_command(
            label="Delete", command=self.delete_selected_record
        )
        self.context_menu.add_command(label="Refresh", command=self.update_treeview)
        ...
        self.nickname_label = tk.Label(self.input_frame, text=f"Ник: {self.nickname}")
        self.nickname_label.grid(row=0, column=0, padx=0, pady=0, sticky="w")

        self.ip_label = tk.Label(self.input_frame, text=f"Ip: {self.client.ip}")
        self.ip_label.grid(row=1, column=0, padx=0, pady=0, sticky="w")

        self.port_label = tk.Label(self.input_frame, text=f"Port: {self.client.port}")
        ...
        self.refresh_button = tk.Button(
            self.db_frame, text="Add user", command=self.on_add_user, width=10
        )
        ...
        self.change_nickname_button = tk.Button(
            self.db_frame, text="Изменить ник", width=12, command=self.change_nickname
        )
        self.change_nickname_button.grid(row=2, column=0, padx=10, pady=10, sticky="w")
        ...
        try:
            ...
            if self.client.connect(ip, int(port)):
                self.open_chat_window(
                    ip, self.client.clients_socket[self.client.get_ind_by_ip(ip)]
                )
            new_window.destroy()
        except TimeoutError:
    ...
    def open_add_user_window(self):
        ...
        tk.Label(add_user_window, text="Nickname:").grid(
        ...
        ok_button = tk.Button(
            ...
            command=lambda: self.add_user(
                nickname_entry.get(), ip_entry.get(), port_entry.get(), add_user_window
            ),
        )
        ...
        cancel_button = tk.Button(
            add_user_window, text="Cancel", command=add_user_window.destroy
        )
    ...
    def validate_ipv4(ip):
        ipv4_pattern = (
            r"\b([1-9]\d{0,2})\.([1-9]?\d{1,2})\.([1-9]?\d{1,2})\.([1-9]?\d{0,2})\b"
        )
...
class ChatWindow:
    def __init__(self, root, socket, nick):
        ...
        self.sender = sender.Sender(socket)
        ...
        self.send_button = tk.Button(
            self.input_frame, text="Отправить", command=self.send_text
        )
        self.send_button.grid(row=0, column=1, padx=5, pady=5)

        # Кнопка для добавления файла
        self.add_file_button = tk.Button(
            self.input_frame, text="Add File", command=self.open_file_dialog
        )
    ...
    def display_text(self, text, nickname):
        ...
        self.text_area.insert(tk.END, f"{nickname}: {text}\n")
        self.text_area.config(state=tk.DISABLED)
...
class WelcomeWindow:
    def __init__(self, root):
        ...
        self.label = tk.Label(self.root, text="Введите ник", font=("Arial", 16))
        self.label.pack(pady=20)

        # Поле для ввода текста (никнейм)
        self.nickname_entry = tk.Entry(self.root)
        self.nickname_entry.pack(pady=10)
        self.nickname_entry.bind("<KeyRelease>", self.check_entry)  # Привязка к событию
